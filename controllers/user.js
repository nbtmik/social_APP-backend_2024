...
exports.followUser = async (req, res) =>{
    try{
        const userToFollow = await User.findById(req.params.id); // to find user to whom u want to follow
        const loggedInUser = await User.findById(req.user._id); // to find our id
...
exports.deleteMyProfile = async (req,res) =>{
    try{
        ...
        for(let i=0;i<posts.length;i++){
            ...
            //Removing post from cloudinary
            await cloudinary.v2.uploader.destroy(post.image.public_id);
            await post.deleteOne();
        }

        //Removing User from followers list after deleting the account
        ...
        //Removing User from following list after deleting the account
...
exports.getAllUsers = async(req,res)=>{
    try{
        ...
        }); //to search operation (options is used for case sensative) and regex used to search pattern not for exact match
...
exports.forgotPassword = async (req, res) => {
    try {
      ...
      await user.save();
      const resetUrl = `${process.env.FRONTEND_URL}/password/reset/${resetPasswordToken}`; // link in which the user will click which will send in email
